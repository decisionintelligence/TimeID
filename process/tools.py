...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, logpath, epoch, filename):
        score = -val_loss
        if self.best_score is None:
            self.best_score = score
            self.save_checkpoint(val_loss, model, logpath, epoch, filename)
    ...
    def save_checkpoint(self, val_loss, model, logpath, epoch, filename):
        if self.verbose:
...
def test_params_flop(model,x_shape):
    """
    If you want to thest former's flop, you need to give default value to inputs in layers.forward(), the following code can only pass one argument to forward()
    """
    model_params = 0
