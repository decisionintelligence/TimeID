...
class DBC(nn.Module):
    ...
    def test_time_tuning(self, model, inputs, batch_x_mark, dec_inp, batch_y_mark, optimizer, cfg, target_output):
        target_output = target_output.cuda()
        for j in range(cfg.TTA_STEPS):
            with torch.cuda.amp.autocast():
                output = model(inputs, j)
                #output = model(inputs, batch_x_mark, dec_inp, batch_y_mark)
                iic_loss = self.criterion(output, target_output)
            optimizer.zero_grad()
            iic_loss.backward()
            optimizer.step()
        return output

    def test_time_adapt_eval(self, input, batch_x_mark, dec_inp, batch_y_mark, model, optimizer, cfg, target_output):
        output = self.test_time_tuning(model, input, batch_x_mark, dec_inp, batch_y_mark, optimizer, cfg, target_output)
        with torch.no_grad():
            with torch.cuda.amp.autocast():
                model.eval()
                output = model(input[:, -cfg.seq_len:, :], 0)
                #output = model(input[:, -cfg.seq_len:, :], batch_x_mark[:, -cfg.seq_len:, :], dec_inp, batch_y_mark)
                output = output[:, -cfg.pred_len:, :]
        return output
    ...
    def train_forward_closed(self, args, x, batch_x_mark, dec_inp, batch_y_mark, y, epoch, model_ViL, model_ViL_optim, outputs_source_list, idx):
        ...
        with torch.no_grad():
            outputs_target = x_pred.clone().detach()
        
        outputs_FPT = self.test_time_adapt_eval(x, batch_x_mark, dec_inp, batch_y_mark, model_ViL, model_ViL_optim, args, outputs_target)

        b, l, source_dim = outputs_source_list[idx % len(outputs_source_list)].shape
        source_proj_layer = nn.Linear(source_dim, self.emb_dim).to(args.device)

        with torch.no_grad():
            outputs_source = outputs_source_list[idx % len(outputs_source_list)].cuda()
            if outputs_source.shape[2] != outputs_target.shape[2]:
                outputs_source = source_proj_layer(outputs_source.reshape(b * l, source_dim))
                outputs_source = outputs_source.reshape(b, l, -1)
            new_FPT = (outputs_target - 1*outputs_source) + outputs_FPT.cuda()
            #new_FPT = outputs_FPT.cuda()

        iic_loss = self.criterion(x_pred, new_FPT)
        loss += args.IIC_PAR * iic_loss
    ...
    def forward(self, args, x, y, epoch, model_ViL=None, model_ViL_optim=None, outputs_source_list=None, idx=None, batch_x_mark=None, dec_inp=None, batch_y_mark=None):
        if self.training:
            loss, pred = self.train_forward(args, x, batch_x_mark, dec_inp, batch_y_mark, y, epoch, model_ViL, model_ViL_optim, outputs_source_list, idx)
        else:
            with torch.no_grad():
